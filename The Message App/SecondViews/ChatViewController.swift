...
class ChatViewController: JSQMessagesViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate, IQAudioRecorderViewControllerDelegate {
    ...
    override func didPressSend(_ button: UIButton!, withMessageText text: String!, senderId: String!, senderDisplayName: String!, date: Date!) {
        ...
        if text != "" {
            ...
            let audioVC = AudioViewController(delegte_: self)
    ...
    //for multimedia meaasages delete option
        ...
        if let audioPath = audio {
            uploadAudio(autioPath: audioPath, chatRoomId: chatRoomId, view: self.navigationController!.view) { (audioLink) in
                ...
                if audioLink != nil {
                    ...
                    outgoingMessage = OutgoingMessage(message: encryptedText, audioLink: audioLink!, senderId: currentUser.objectId, senderName: currentUser.firstname, date: date, status: kDELETED, type: kAUDIO)
    ...
    func loadMessages(){
        ...
        updatedListener = reference(.Message).document(FUser.currentId()).collection(chatRoomId).addSnapshotListener({ (snapshot, error) in
